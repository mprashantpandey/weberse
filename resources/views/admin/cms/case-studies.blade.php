@extends('layouts.dashboard', [
    'title' => 'CMS Case Studies',
    'heading' => 'Case Studies',
    'subheading' => 'Manage case studies through a dedicated table and form workflow.',
    'nav' => [
...
@section('content')
    ...
    <div class="card">
        <div class="flex items-center justify-between gap-4">
            <div>
                <div class="panel-title">All Case Studies</div>
                <div class="panel-subtitle">Create and edit case-study detail pages from dedicated forms.</div>
            </div>
            <a href="{{ route('admin.cms.case-studies.create') }}" class="btn-primary">Create Case Study</a>
        </div>
                ...
                <tbody>
                    @foreach($caseStudies as $item)
                        <tr>
                            ...
                            <td>{{ $item->client ?: '—' }}</td>
                            <td>{{ $item->industry ?: '—' }}</td>
                            <td><span class="status-badge">{{ $item->is_published ? 'Visible' : 'Hidden' }}</span></td>
                            <td class="text-right"><a href="{{ route('admin.cms.case-studies.edit', $item) }}" class="btn-dark px-4 py-2 text-xs">Manage</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
